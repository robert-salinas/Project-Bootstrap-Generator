...
def init(
    ...
    project_type: str = typer.Option("python_cli", "--type", "-t", help="Tipo de proyecto (python_cli, python_web, node_cli, hardware_esp32)"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Ruta de salida (por defecto el nombre del proyecto)"),
):
    """
    Inicializa un nuevo proyecto basado en una plantilla.
    """
    if not validate_project_name(project_name):
        console.print(f"[bold red]Error:[/bold red] El nombre '{project_name}' no es válido. Use solo letras, números, guiones y guiones bajos.")
        raise typer.Exit(code=1)

    target_path = output if output else Path.cwd() / project_name

    if not validate_output_path(target_path):
        console.print(f"[bold red]Error:[/bold red] La ruta '{target_path}' ya existe y no está vacía.")
        raise typer.Exit(code=1)

    generator = ProjectGenerator()
    
    try:
        with console.status(f"[bold green]Generando proyecto {project_type} en {target_path}..."):
